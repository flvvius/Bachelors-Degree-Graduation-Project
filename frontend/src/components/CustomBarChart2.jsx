...
const CustomBarChart2 = () => {
        ...
        const fetchTasks = async () => {
            ...
            try {
                const tasksPromises = selectedUsers.map(user =>
                    axios.get(`http://localhost:8080/api/task/getTasksByUser/${user.value}`)
                );
    ...
    return (
        <Box width="100%" p={5} bg={bgColor} borderRadius="lg" boxShadow="lg">
            <Text fontSize="2xl" mb={4} textAlign="center" fontWeight="bold" color={textColor}>Total Number of Tasks Each Employee Finished by Month</Text>
            <Box mb={5}>
                ...
                <Select
                    ...
                    styles={{
                        ...
                        option: (provided, state) => ({
                            ...
                            color: state.isSelected ? textColor : state.isFocused ? textColor : textColor
                        })
                    }}
                />
            </Box>
